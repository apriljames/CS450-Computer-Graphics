...
//		7. The view to be changed from "Inside" (the cockpit) and "Outside" (entire scene)
...
const char *WINDOWTITLE = { "Animate a Helicopter- April James" };
...
int		ViewType;				// != 0 means to change the view type from inside/outside
GLuint	BladesList;				// object display list for helicopter blades
GLuint	HeliList;				// object display list for helicopter
...
float	BladeRot;				// rotation angles in degrees for helicopter blade
bool	Frozen;					// toggles animation on/off

const float BLADESPEED = { 5.f };	// value to increase the speed of rotating blades
...
Display( )
{
	...
	glColor3f(183. / 255., 214. / 255., 120. / 255.);
	glCallList(HeliList);

	// draw the helicopter blades:
	glPushMatrix();
		glColor3f(255./255., 165. / 255., 120. / 255.);
	...
	glPushMatrix();
		glColor3f(255. / 255., 213. / 255., 122. / 255.);
		glTranslatef(.5, 2.5, 9.);
		glRotatef(BladeRot*3, 1., 0., 0.);
		glRotatef(90., 0., 1., 0.);
	...
	// draw the reference object in front of the helicopter ( a teapot ):
	glPushMatrix();
		glColor3f(186./255., 1., 227./255.);
...
InitLists( )
{
	...
	//create the helicopter blades object:
