...
GLuint	SpiralList;				// spiral display list
...
int		colorChoice;			// a selector to change color combinations in the display list
...
Animate( )
{
	...
	glutSetWindow( MainWindow );
	glutPostRedisplay( ); //forces Display() func. to be called to redraw scene with new display param values
}
...
Display( )
{
	...
	//double clipEq[2][4] =
	//	{ { radius, 1.0, 0.0, 0.0 }, { -radius, 0.0, 0.0, 1.0 } };
...
InitGraphics( )
{
	...
	//all GLUT_XXX constants are defined in glut.h
...
InitLists( )
{
	...
	float radius = 1.0f;
	float steps = 100.;
	float angle = 2.f * 3.14159265  / steps;		//circumference / # steps
	...
	glNewList(SpiralList, GL_COMPILE);
			...
			for (float i = 0.; i <= 5*steps; i+=1.) {
				if (colorChoice == 1)
					glColor3f(0., 1. - i/500., 0. + i/500);
				else if (colorChoice == 2)
					glColor3f(1. - i / 500., 0. + i / 500, 0.);
				else 
					glColor3f(1. - i / 500., 0., 0. + i / 500);
				newx = radius * sin(angle * i);
				newy = -radius * cos(angle * i);
				
					glVertex3f(0., 0., i/100);
					glVertex3f(prevx, prevy, i/100);
					//glVertex3f(newx, newy, ((i+1.)/2.)/100.);
				
				glVertex3f(newx, newy, i / 100);
				prevx = newx; 
				prevy = newy;
			}
			glEnd();
