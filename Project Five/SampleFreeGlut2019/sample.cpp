...
const char *WINDOWTITLE = { "OpenGL Final Project: April James" };
...
float	XPan = 0.;				// used for movement panning with arrow keys 
float	YPan = 0.;				// used for movement panning with arrow keys 
float	ZPan = 0.;				// used for movement panning with arrow keys
...
Display( )
{
	...
	gluLookAt((XPan), (0.), (ZPan + 3.),		(XPan), (0.), (ZPan),		0., 1., 0. );
...
// keyboard callback for special keys (e.g. arrow keys)
void SpecialKey(int key, int x, int y)
{
	...
	case GLUT_KEY_UP:
		if (DebugOn != 0)
			fprintf(stderr, "SpecialInput: %d, %d, %d\n", key, x, y);
		ZPan += 1;
		break;
	case GLUT_KEY_DOWN:
		if (DebugOn != 0)
