...
GLSLProgram*	Pattern;		
bool			AnimateVertexShader = false;
bool			AnimateFragmentShader = false;
...
//display object related variables
GLuint			SphereList;
...
Animate( )
{
	...
	Time = (float)ms / (float)MS_IN_THE_ANIMATION_CYCLE * 2 * PI;        // [ 0., 1. )
...
Display( )
{
	...
	// selecting shader vs full pipeline

	Pattern->Use();
	TimeVert = 0;
	if (AnimateVertexShader) {
		TimeVert = Time;
	}
	Pattern->SetUniformVariable("uTimeVert", TimeVert);

	TimeFrag = 0;
	float uShininessFrag = 30;
	float uSpecularColor[] = { 1.,1.,1. };
	float uKa = .5;
	float uKd = .5;
	float uKs = .5;

	if (AnimateFragmentShader) {
		TimeFrag = Time;
	}
	Pattern->SetUniformVariable("uTimeFrag", TimeFrag);
	Pattern->SetUniformVariable("uShininess", uShininessFrag);
...
InitGraphics( )
{
	...
	Pattern = new GLSLProgram(); //create shader program
	bool valid = Pattern->Create("pattern.vert", "pattern.frag"); //setting up shaders
	if (!valid)
	{
...
InitLists( )
{
	...
	glNewList(SphereList, GL_COMPILE);
		OsuSphere(SphereRad, slices, stacks);
	glEndList();
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 'V':
		case 'v':
			AnimateVertexShader = true;
			AnimateFragmentShader = false;
			break;

		case 'F':
		case 'f':
			AnimateVertexShader = false;
			AnimateFragmentShader = true;
			break;
